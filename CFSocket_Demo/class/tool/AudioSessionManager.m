...
static AudioSessionManager *_udioSessionMgr = nil;
+ (instancetype)sharedInstance {
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _udioSessionMgr = [[self alloc] init];
    });
    return _udioSessionMgr;
}
...
- (BOOL)configureAudioSessionWithDesiredAudioRoute:(NSString *)desiredAudioRoute
{
    ...
    NSLog(@"self.currentSetRoute: %@", self.currentSetRoute);
	return YES;
}
...
- (void)currentRouteChanged:(NSNotification *)notification
{
    ...
    NSString *oldOutput = nil;
    if (oldRoute.outputs.count>0) {
        oldOutput = [[oldRoute.outputs objectAtIndex:0] portType];
    }
    AVAudioSessionRouteDescription *newRoute = [audioSession currentRoute];
    NSString *newOutput = nil;
    if (newRoute.outputs.count > 0) {
        newOutput = [[newRoute.outputs objectAtIndex:0] portType];
    }
    NSLog(@"\ncurrentRouteChanged changeReason = %ld\n {oldOutput = %@, newRoute = %@}\n", (long)changeReason, oldOutput, newOutput);
    switch (changeReason) {
...
- (void)setAudioRouteWithSpeaker:(BOOL)speaker
{
    ...
    if (speaker) {
        ...
        if (![[AVAudioSession sharedInstance] overrideOutputAudioPort:AVAudioSessionPortOverrideSpeaker error:&err]) {
            NSLog(@"unable to overrideOutputAudioPort: %@", err);
            return;
        };
    } else {
        mMode = kAudioSessionManagerMode_Record;
    ...
    // Display our current route...
    NSLog(@"current route: %@", self.audioRoute);
    NSLog(@"res = %d", res);
}
...
{
    NSLog(@"desiredAudioRoute = %@", desiredAudioRoute);
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
	NSError *err;
    ...
    else if ( desiredAudioRoute == kAudioSessionManagerDevice_Headset || desiredAudioRoute == kAudioSessionManagerDevice_Phone ) { // 麦耳机(苹果叫Headset，不带麦耳机叫Headphone
        if ( [audioSession respondsToSelector:@selector(overrideOutputAudioPort:error:)] ) {
            // replace AudiosessionSetProperty (deprecated from iOS7) with AVAudioSession overrideOutputAudioPort
    ...
    NSLog(@"current audioRoute = %@", self.audioRoute);
    return YES;
}
